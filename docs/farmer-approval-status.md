...
| Migration | Nội dung |
|---|---|
| `V20260925003__create_farmer_profiles_table.sql` | Bảng `farmer_profiles`: `user_id` UNIQUE FK, `stall_name`, `contact_person`, `approval_status`, `approved_by`, `approved_at`, timestamps |
| `V20260925004__extend_farmer_profiles_for_full_application.sql` | Thêm các cột của form prototype: `description`, `categories`, `main_crops`, `weekly_volume`, `growing_method`, `plot_address`, `plot_size`, `growing_since_year`, `plot_latitude/longitude`, `photo_paths`, `video_path`, `preferred_market_name` |
| `V20260925005__add_reject_reason_to_farmer_profiles.sql` | Cột `reject_reason` |
