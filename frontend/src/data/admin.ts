...
/**
 * What the platform charges a stall. None of this is in the SRS — the requirements rule out a payment gateway — so
 * every screen that reads it carries that warning. Prices are in đồng, sized against a stall that takes roughly 200,000
 * ₫ on a market morning.
 */
export type PricingExtra = { key: 'listing' | 'bump'; name: string; price: number; sold: number };

export type PricingPin = {
  key: 'market' | 'home';
  name: string;
  slots: number;
  note: string;
  /** [days, price] for the three lengths a stall can buy. */
  days: [number, number][];
};
...
export const pricing = {
  ...
  extras: [
    { key: 'listing', name: 'One listing past the daily five', price: 2000, sold: 128 },
    { key: 'bump', name: 'One bump past the monthly ten', price: 3000, sold: 61 },
  ] as PricingExtra[],
  pins: [
    {
      key: 'market',
  ...
  /** Tiles above the price list. */
  stats: { payingStalls: 5, approvedStalls: 8, pinnedDaysSold: 34, pinnedDaysAvailable: 150, stallsOverFreeLimit: 2 },
};

/**
 * What MarketLink itself earns, separate from the money customers hand to stalls — the platform never touches that.
 * Sources add up to `total`; bundles are credit, so they are counted where the credit is spent rather than as a fifth
 * source.
 */
export type RevenueSource = { name: string; amount: number; prev: number; sold: string; stalls: number };

export type RevenueStall = { stall: string; spent: number; share: string; orders: number };

export const platformRevenue = {
  total: 2229000,
  totalPrev: 1656000,
  /** What the markets took. A different number, never added to the one above. */
  gmv: 31900000,
  gmvPrev: 27690000,
  payingStalls: 5,
  payingStallsPrev: 4,
  approvedStalls: 8,
  dailyNow: [
    146000, 127000, 151000, 137000, 69000, 15000, 27000, 40000, 112000, 64000, 210000, 10000, 37000, 98000, 10000,
    92000, 49000, 194000, 49000, 96000, 136000, 3000, 53000, 14000, 65000, 2000, 112000, 10000, 87000, 14000,
  ],
  dailyPrev: [
    21000, 56000, 5000, 137000, 28000, 26000, 6000, 19000, 21000, 32000, 81000, 20000, 150000, 7000, 13000, 17000,
    111000, 89000, 82000, 18000, 23000, 24000, 59000, 69000, 183000, 83000, 64000, 145000, 19000, 48000,
  ],
  sources: [
  ...
  /** Short labels for the source-by-source chart, where the full names do not fit. */
  sourceShort: ['Market pin', 'Home pin', 'Listings', 'Bumps'],
  byStall: [
